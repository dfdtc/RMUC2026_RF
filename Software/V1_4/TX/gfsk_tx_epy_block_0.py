# This work is based on an unlicensed open source project from: SCNU-PIONEER
# From repositorie: https://github.com/SCNU-PIONEER/PinyRadio
# All copyright and credit belongs to the original author
# The code is used here for educational and research purposes only
...
class byte_vector_source(gr.sync_block):
    ...
    def __init__(self, sample_rate = 1e6, payload=None, cmd_option="enemy_pos", seq=-1,chunk_bits = 1, pad_mode="pad", mode="local", rand_pad = False, push_freq = 10):
        ...
        if self.mode == "client":
            ...
            if self.rand_pad:
                pad_num = self.push_period * sample_rate - len(self.vector)
                np.append(self.vector,list(random.randbytes(pad_num)))
    ...
    def _build_frame(self, payload, cmd_option=None):
        ...
        seq_derived = self._seq_manual & 0xFF
    ...
    def _bytes_to_chunks(self, byte_arr, chunk_bits=2):
        ...
        """
        if chunk_bits <= 0:
            raise ValueError("chunk_bits must >=1")
