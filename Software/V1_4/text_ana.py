...
def bf_header_fixing(data_len, data, crc, fix_mode="key"):
    ...
    if data_len == 1030:
        ...
        for i in range(255):
            data_temp[3] = i
            if header_crc_calc.checksum(bytes(data_temp)) == crc:
                return data_temp
        return None
...
while i < n:
    ...
    # 读取数据长度 (2 字节小端)
    len_bytes = temp[start+1 : start+3]
    data_len = int.from_bytes(len_bytes, byteorder='little')
    seq = temp[start+3]
    header_crc8 = temp[start+4]
    ...
    if header_crc_calc.checksum(bytes(header_list)) != header_crc8:
        ...
        header_broken += 1
        # 暴力修复（传入副本）
        fixed = bf_header_fixing(data_len, list(header_list), header_crc8)
        if fixed is None:
    ...
    cmd = int.from_bytes(temp[cmd_start:cmd_start+2], byteorder='little')
    data = temp[data_start : data_start + data_len]
    frame_crc16 = int.from_bytes(temp[crc_start : crc_start+2], byteorder='little')

    # 构建帧体（不含尾部 CRC16）
    frame_body = temp[cmd_start : crc_start]   # cmd(2) + data(data_len)
    # 完整帧 = header_list（5字节） + frame_body
    full_frame = bytes(header_list + frame_body)

    # ---------- 5. 帧 CRC16 校验 ----------
    if frame_crc_calc.checksum(full_frame) != frame_crc16:
        frame_broken += 1
